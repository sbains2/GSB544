...
Github Link: https://github.com/sbains2/GSB544
...
```
Process for rebuilding the plots:
- Filtering: Subsetted the data so it's only from the year 2010 with a valid region so that the legend or the plot doesn't include 'nan'
- Scales: To match the gapminder spread, I used the scale_x_continuous() function with custom breaks and labels.
- Colors: In order to match the color palette of the gapminder graph, I specified a manual palette via the scale_fill_manual()
- Annotation: I added the annotation line in order to incldue the year in the backdrop to match the gapminder plot
...
```
Process for Rebuilding the plot:
- Type fix: I transformed energy to numeric as it was showing non-numeric for some rows
- Filtering: I called dropna() so that rows that had missing mapped aesthetics don't get dropped by the layer
...
```
Process for rebuilding the plot:
- Scales: Y: Used the scale_y_continuous(trans=log10) in order to visualize the low and high GDP countries
